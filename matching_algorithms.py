	...
	for p_id in range(nplayers):
		...
		if p_id in payoffs.keys():
			...
			for sm_id in sm_pay_dict.keys():
				p[sm_id] = sm_pay_dict[sm_id]		# this does not seem to be happening
		matrix.append(p)
...
def fair_matching(payoffs, nslots, nplayers):
	...
	cost = np.multiply(-1,payoff)

	row_ind, col_ind = linear_sum_assignment(cost)

	noise = np.random.normal(0.0, 3.0, size=nslots)
	for i in range(len(row_ind)):
		noise[i] += (payoff[row_ind[i], col_ind[i]])

	pays = normalize_payoff(noise)
...
def self_matching(payoffs, nslots, nplayers, q):
	...
	for i in range(len(player_ind)):
		noise[i] += (payoff[player_ind[i], sm_ind[i]])
...
def normalize_payoff(p):
	...
	return p_norm.astype(int, copy=False)
